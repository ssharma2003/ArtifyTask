...
const ProjectDetails = ({ route }) => {
  ...
  useEffect(() => {
    const fetchProject = async () => {
      try {
        const response = await axios.get(`http://localhost:2000/api/projects/${projectId}`);
  ...
  return (
    <View style={styles.container}>
      ...
      {assignedEmployees.length > 0 ? (
        assignedEmployees.map(emp => (
          <Text key={emp._id} style={styles.listItem}>- {emp.name}</Text>
        ))
      ) : (
        <Text style={styles.noDataText}>No employees assigned.</Text>
      )}
      ...
      {assignedEquipment.length > 0 ? (
        assignedEquipment.map(eq => (
          <Text key={eq._id} style={styles.listItem}>- {eq.name}</Text>
        ))
      ) : (
        <Text style={styles.noDataText}>No equipment assigned.</Text>
      )}
